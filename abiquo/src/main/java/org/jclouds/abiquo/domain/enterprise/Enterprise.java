...
public class Enterprise extends DomainWithLimitsWrapper<EnterpriseDto> {
	...
	public static class Builder extends LimitsBuilder<Builder> {
		...
		public Builder pricingTemplate(final PricingTemplate pricingTemplate) {
		...
		public Enterprise build() {
			...
			if ( pricingTemplate != null ) {
				dto.addLink(new RESTLink("pricingtemplate", pricingTemplate.getURI().toString()));
			}
		...
		public static Builder fromEnterprise(final Enterprise in) {
			return Enterprise.builder(in.context).name(in.getName())
					...
					.pricingTemplate(in.getPricingTemplate().isPresent()? in.getPricingTemplate().get(): null);
		}
	}

	public Optional<PricingTemplate> getPricingTemplate() {
		Integer pricingTemplateId = target.getIdPricingTemplate();
		if ( pricingTemplateId != null ) {
			return Optional.of(wrap(context, PricingTemplate.class, context.getApi().getConfigApi().getPricingTemplate(pricingTemplateId)));
		}
